export const fakeMovies = [
  {
    idmovies: 1,
    name: "VENOM: KÈO CUỐI",
    genre: "Hành Động",
    duration: "103",
    release_date: "25/10/2024",
    actor:
      "Tom Hardy, Juno Temple, Chiwetel Ejiofor, Clark Backo, Stephen Graham",
    director: "Kelly Marcel",
    language: "Tiếng Anh - Phụ đề Tiếng Việt",
    title: "venom-3",
    description:
      "Đây là phần phim cuối cùng và hoành tráng nhất về cặp đôi Venom và Eddie Brock (Tom Hardy). Sau khi dịch chuyển từ Vũ trụ Marvel trong ‘Spider-man: No way home’ (2021) trở về thực tại, Eddie Brock giờ đây cùng Venom sẽ phải đối mặt với ác thần Knull hùng mạnh - kẻ tạo ra cả chủng tộc Symbiote và những thế lực đang rình rập khác. Cặp đôi Eddie và Venom sẽ phải đưa ra lựa quyết định khốc liệt để hạ màn kèo cuối này.",
    image_url:
      "https://bhdstar.vn/wp-content/uploads/2024/10/referenceSchemeHeadOfficeallowPlaceHoldertrueheight700ldapp-21.jpg",
    rated: "P - PHIM DÀNH CHO MỌI ĐỐI TƯỢNG",
    trailer: "https://www.youtube.com/watch?v=id1rfr_KZWg",
  },
  {
    idmovies: 2,
    name: "NGÀY XƯA CÓ MỘT CHUYỆN TÌNH",
    genre: "Tình cảm",
    duration: "103",
    release_date: "25/10/2024",
    actor: "Avin Lu, Ngọc Xuân, Đỗ Nhật Hoàng, Thanh Tú, Bảo Tiên, Hạo Khang",
    director: "Trịnh Đình Lê Minh",
    language: "Tiếng Việt - Phụ đề Tiếng Anh",
    title: "nxcmct",
    description:
      "Ngày Xưa Có Một Chuyện Tình xoay quanh câu chuyện tình bạn, tình yêu giữa hai chàng trai và một cô gái từ thuở ấu thơ cho đến khi trưởng thành, phải đối mặt với những thử thách của số phận. Trải dài trong 4 giai đoạn từ năm 1987 - 2000, ba người bạn cùng tuổi - Vinh, Miền, Phúc đã cùng yêu, cùng bỡ ngỡ bước vào đời, va vấp và vượt qua.",
    image_url:
      "https://bhdstar.vn/wp-content/uploads/2024/10/referenceSchemeHeadOfficeallowPlaceHoldertrueheight700ldapp-13.jpg",
    rated: "T16 - Phim được phổ biến đến người xem từ đủ 16 tuổi trở lên (16+)",
    itemRate: "t16",
    trailer: "https://www.youtube.com/watch?v=qaeHlk0OXec",
  },
  {
    idmovies: 6,
    name: "CÁM",
    genre: "Thần thoại",
    duration: "103",
    release_date: "20/09/2024",
    actor: "Quốc Cường, Thúy Diễm, Rima Thanh Vy, Lâm Thanh Mỹ, Hải Nam",
    director: "Kelly Marcel",
    language: "Tiếng Anh - Phụ đề Tiếng Việt",
    title: "cam",
    description:
      "Câu chuyện phim là dị bản kinh dị đẫm máu lấy cảm hứng từ truyện cổ tích nổi tiếng Tấm Cám, nội dung chính của phim xoay quanh Cám - em gái cùng cha khác mẹ của Tấm đồng thời sẽ có nhiều nhân vật và chi tiết sáng tạo, gợi cảm giác vừa lạ vừa quen cho khán giả.",
    image_url:
      "https://bhdstar.vn/wp-content/uploads/2024/09/referenceSchemeHeadOfficeallowPlaceHoldertrueheight700ldapp-9.jpg",
    rated: "T18 - Phim được phổ biến đến người xem từ đủ 18 tuổi trở lên (18+)",
    trailer: "https://www.youtube.com/watch?v=_8qUFEmPQbc",
  },
  {
    idmovies: 7,
    name: "RED ONE: MẬT MÃ ĐỎ",
    genre: "Thần thoại",
    duration: "123",
    release_date: "08/11/2024",
    actor: "Chris Evans, Dwayne Johnson, Lucy Liu",
    director: "Jake Kasdan",
    language: "Phụ đề/Lồng tiếng",
    title: "red-one-mat-ma-do",
    description:
      "Sau khi Ông già Noel (tên mã: Red One) bị bắt cóc, Trưởng phòng An ninh Bắc Cực (Dwayne Johnson) phải hợp tác với thợ săn tiền thưởng khét tiếng nhất thế giới (Chris Evans) trong một nhiệm vụ hành động vòng quanh thế giới để cứu Giáng Sinh.",
    image_url:
      "https://bhdstar.vn/wp-content/uploads/2024/11/referenceSchemeHeadOfficeallowPlaceHoldertrueheight700ldapp-14.jpg",
    rated:
      "K - Phim được phổ biến đến người xem dưi 13 tuổi với điều kiện xem cùng cha, mẹ hoặc người giám hộ",
    trailer: "https://www.youtube.com/watch?v=2T_mKyH17mY",
  },
  {
    idmovies: 8,
    name: "OZI: VOICE OF THE FOREST: PHI VỤ RỪNG XANH",
    genre: "Thần thoại",
    duration: "87",
    release_date: "20/09/2024",
    actor: "Marissa Anita, Kemah Bob",
    director: "Tim Harper",
    language: "Tiếng Anh - Phụ đề Tiếng Việt",
    title: "ozi-voice-of-the-forest-phi-vu-rung-xanh",
    description:
      "Câu chuyện về Ozi, một chú đười ươi mồ côi sử dụng kỹ năng gây ảnh hưởng của mình để cứu khu rừng và ngôi nhà của mình khỏi nạn phá rừng.",
    image_url:
      "https://bhdstar.vn/wp-content/uploads/2024/11/referenceSchemeHeadOfficeallowPlaceHoldertrueheight700ldapp.jpg",
    rated: "P - PHIM DÀNH CHO MỌI ĐỐI TƯỢNG",
    trailer: "https://www.youtube.com/watch?v=cRjzT9OHpLE",
  },
  {
    idmovies: 9,
    name: "MOANA 2: HÀNH TRÌNH CỦA MOANA",
    genre: "Thần thoại",
    duration: "100",
    release_date: "29/11/2024",
    actor: "Auli'i Cravalho, Dwayne Johnson",
    director: " David G. Derrick Jr., Jason Hand, Dana Ledoux Miller",
    language: "Tiếng Anh - Phụ đề Tiếng Việt",
    title: "moana-2-hanh-trinh-cua-moana",
    description:
      "Sau khi nhận được cuộc gọi bất ngờ từ tổ tiên có khả năng chỉ đường, Moana lên đường đến vùng biển xa xôi của Châu Đại Dương và vào vùng nước nguy hiểm đã mất tích từ lâu để thực hiện một cuộc phiêu lưu không giống bất kỳ điều gì cô từng trải qua.",
    image_url:
      "https://bhdstar.vn/wp-content/uploads/2024/10/referenceSchemeHeadOfficeallowPlaceHoldertrueheight700ldapp-2.png",
    rated: "T18 - Phim được phổ biến đến người xem từ đủ 18 tuổi trở lên (18+)",
    trailer: "https://www.youtube.com/watch?v=gqyDX4oB7Pc",
  },
];

// Có thể thêm các fake data khác nếu cần
export const fakeEvents = [];
export const fakePromotions = [];

export const fakeShowtimes = [
  {
    idcinemas: 1,
    name: "BHD Star Phạm Ngọc Thạch",
    address: "Tầng 8, Vincom Phạm Ngọc Thạch, Q.3",
    hall: [
      {
        idhalls: 1,
        name: "Cinema 01",
        showtimes: [
          {
            idshowtimes: 1,
            start_time: "10:15",
            end_time: "12:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 2,
            start_time: "13:15",
            end_time: "15:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 3,
            start_time: "16:15",
            end_time: "18:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 4,
            start_time: "19:15",
            end_time: "21:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 5,
            start_time: "22:15",
            end_time: "24:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
        ],
      },
      {
        idhalls: 2,
        name: "Cinema 02",
        showtimes: [
          {
            idshowtimes: 1,
            start_time: "10:15",
            end_time: "12:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 2,
            start_time: "13:15",
            end_time: "15:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 3,
            start_time: "16:15",
            end_time: "18:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 4,
            start_time: "19:15",
            end_time: "21:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 5,
            start_time: "22:15",
            end_time: "24:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
        ],
      },
    ],
  },
  {
    idcinemas: 2,
    name: "BHD Star Nguyễn Văn Linh",
    address: "Tầng 8, Vincom Nguyễn Văn Linh, Q.3",
    hall: [
      {
        idhalls: 1,
        name: "Cinema 01",
        showtimes: [
          {
            idshowtimes: 1,
            start_time: "10:15",
            end_time: "12:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 2,
            start_time: "13:15",
            end_time: "15:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 3,
            start_time: "16:15",
            end_time: "18:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 4,
            start_time: "19:15",
            end_time: "21:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
          {
            idshowtimes: 5,
            start_time: "22:15",
            end_time: "24:15",
            seatsAvailable: 45,
            isAlmostFull: false,
            isSoldOut: false,
          },
        ],
      },
    ],
  },
];
export const fakeSeatStatus = [
  {
    idshowtimes: 1,
    bookedSeats: ["A2", "A3", "B3", "D5", "D6", "E1", "F4", "I0-1", "I4-5"],
  }
];

export const fakePayment = [
  {
    order_code : "CGV23112013361912",  
    payUrl : "https://test-payment.momo.vn/v2/gateway/pay?t=TU9NT1NKSTEyMDE5MDgyM3wxNjEyODQwNTMyNDI3OjAxMjM0NTY3Nzg="
  }
];

export const fakePaymentCallback = {
    order_code: "",
    status: "success",
    message: "Giao dịch thành công",
    amount: 50000,
    payment_method: "momo",
    transaction_id: "2321321321",
    payment_time: new Date().toISOString()
};

// Thêm delay time cho fake callback
export const FAKE_CALLBACK_DELAY = 3000; // 3 seconds
